    ...
    <div className=''>
      ...
      {project.map((p) => (
        <div className='mt-[50px] mx-[100px] space-y-7'>
          {/* <Link href="/">
            <XIcon className="w-[50px] text-red-600 cursor-pointer" />
          </Link> */}
          <div className='space-y-2'>
            <h1 className='font-Poppins text-5xl'>{p.title}</h1>
            <p className='font-Comfortaa text-xl'>{p.description}</p>

            <button className='font-Poppins bg-black text-white px-4 py-2.5 rounded-full w-max shadow-sm hover:shadow-xl'>
              <Link href='https://www.linkedin.com' target='_black'>
                <div className='flex space-x-2 items-center'>
                  <p className='text-medium'>Github</p>
                  <svg
                    width='20px'
                    height='20px'
                    viewBox='0 0 24 24'
          ...
          <div className='relative xl:h-[500px] xl:w-[900px] lg:h-[450px] lg:w-[800px] md:h-[300px] md:w-[550px] h-[200px] w-[400px] shadow rounded-xl'>
            <Image
              src={show !== undefined ? show : p.images[0]}
              layout='fill'
              objectFit='cover'
          ...
          <div className='flex flex-wrap pb-10'>
            {p.images.map((img) => (
              <div
                className={
                  show == img
                    ? 'relative lg:h-[100px] lg:w-[150px] md:h-[80px] md:w-[120px] h-[50px] w-[90px] rounded-md cursor-pointer bg-gray-200 mr-5 mt-5'
                    : 'relative lg:h-[100px] lg:w-[150px] md:h-[80px] md:w-[120px] h-[50px] w-[90px] shadow hover:shadow-xl rounded-md cursor-pointer mr-5 mt-5'
                }
                onClick={() => setShow(img)}
              >
                <Image
                  ...
                  layout='fill'
                  objectFit='cover'
                  className={show == img ? 'opacity-0' : 'rounded-md'}
                />
              </div>
            ))}
          </div>
          <div className='grid grid-cols-1 lg:grid-cols-2 space-y-10 lg:space-y-0'>
            <div className='space-y-7'>
              <h2 className='font-Poppins text-4xl text-[#A0D995]'>Fitures</h2>
              {p.fitures.map((f) => (
                <div className='space-y-3'>
                  <h3 className='font-Poppins text-2xl'>{f[0]}</h3>
                  <p className='font-Comfortaa text-lg mr-[100px]'>{f[1]}</p>
                </div>
              ))}
            </div>
            <div className='space-y-7'>
              <h2 className='font-Poppins text-4xl text-[#A0D995]'>
                Technologies
              </h2>
              <div className='space-y-3'>
                {p.technologies.map((t) => (
                  <h3 className='font-Poppins text-2xl'>{t}</h3>
                ))}
              </div>
            </div>
          </div>
